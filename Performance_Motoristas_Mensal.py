    ...
    if tipo_analise=='Tipo de Veículo':
        ...
        if selected_rows is not None and len(selected_rows)>0:
            ...
            if selected_rows_2 is not None and len(selected_rows_2)>0:
                ...
                df_veiculo = montar_df_analise_mensal(st.session_state.df_historico, 'Veiculo', veiculo)

                with row4[1]:
    
                    grafico_duas_barras_linha_percentual(df_veiculo, 'ano_mes', 'serviços', 'Serviços', 
                                                        'meta_batida', 'Metas Batidas', 'performance', 'Performance', 
                                                        veiculo)
